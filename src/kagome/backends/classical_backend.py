...
Exposes a classical FF through kagome's ``Calculator`` interface so it can drive
``kagome.integrators.minimize.compress_box`` (and any other Calculator consumer)
...
``kagome.prep.openmm_equilibrate._build_openff_topology``, which validates the
...
from kagome.units import KCAL_PER_KJ, NM_PER_ANGSTROM
...
class ClassicalCalculator(Calculator):
    """OpenMM+OpenFF Sage energies/forces behind the kagome Calculator API."""
    ...
    def _build(self, species: list[str], cell: NDArray[np.floating] | None) -> None:
        ...
        from kagome.prep.openmm_equilibrate import _build_openff_topology, _make_system
