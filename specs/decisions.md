...
- **nylon 固有ガード**: `run_nylon66.py` の init_bonds 抽出は best-effort(失敗時 warning で継続)だが、混合はトポロジ必須のため **`--mix` 指定時は抽出失敗を即時エラー**とする(wf.run 深部での fail-fast より原因が明確)。epoxy 側にも同じ best-effort 経路があり同じガードを適用。
- **summary.json への混合 provenance 追記(追補)**: vinyl スクリプトが書く `mixing_enabled`/`mix_ps`/`mixing_skipped_cycles`/`n_mixing_skipped`(WM-P4 de-clash 追補の測定汚染赤信号カウンタ)が nylon/epoxy の summary.json に無かったため、同一フィールドを両スクリプトにも追記する。paper-scale 混合ランの監視(スキップ率の赤信号判定)に必須。
